...
void Transmogrification::LoadConfig(bool reload)
{
    ...
    UseGlobalCatalog = sConfigMgr->GetOption<bool>("Transmogrification.Zoe.GlobalCatalog.Enable", false);
    GlobalCatalogSource = sConfigMgr->GetOption<uint32>("Transmogrification.Zoe.GlobalCatalog.Source", 1);
    GlobalCatalogMaxItems = sConfigMgr->GetOption<uint32>("Transmogrification.Zoe.GlobalCatalog.MaxItems", 0);
...
void Transmogrification::LoadCollections()
{
    ...
    if (sTransmogrification->GetUseGlobalCatalog())
    {
        LOG_INFO("module", "Loading ZoeCore transmog global catalog cache....");

        uint32 loaded = 0;
        uint32 maxItems = sTransmogrification->GetGlobalCatalogMaxItems();
        uint32 source = sTransmogrification->GetGlobalCatalogSource();

        std::string query;

        if (source == 2)
        {
            query = "SELECT `item_entry` FROM `custom_transmog_catalog` WHERE `enabled` = 1 ORDER BY `category`, `item_entry`";
            if (maxItems > 0)
                query += " LIMIT " + std::to_string(maxItems);
        }
        else
        {
            // Source 1: auto catalog from world.item_template.
            // class 2 = weapon, class 4 = armor.
            // InventoryType exclusions:
            // 0 non equip, 18 bag, 19 tabard, 20 robe duplicate, 24 ammo, 27 quiver, 28 relic.
            uint32 minItemLevel = sConfigMgr->GetOption<uint32>("Transmogrification.Zoe.GlobalCatalog.MinItemLevel", 60);
            ...
            query =
                ...
                "AND `ItemLevel` >= " + std::to_string(minItemLevel) + " "
                "ORDER BY `entry` ASC";
        ...
        if (QueryResult result = WorldDatabase.Query(query.c_str()))
        {
            do
            {
                ...
                globalCatalogCache.push_back(itemId);
                ++loaded;

            } while (result->NextRow());
        }

        LOG_INFO("module", "Loaded {} ZoeCore global transmog catalog appearances", loaded);
    }
}
...
bool Transmogrification::GetUseGlobalCatalog() const
{
    return UseGlobalCatalog;
}

uint32 Transmogrification::GetGlobalCatalogSource() const
{
    return GlobalCatalogSource;
}

uint32 Transmogrification::GetGlobalCatalogMaxItems() const
{
    return GlobalCatalogMaxItems;
}
