...
std::string GetZoeBreakingNewsPath()
{
    std::string activePage = sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.ActivePage", "news");

    if (activePage == "pvp")
        return sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.PvpPath", "./news/pvp.html");

    if (activePage == "events")
        return sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.EventsPath", "./news/events.html");

    if (activePage == "maintenance")
        return sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.MaintenancePath", "./news/maintenance.html");

    if (activePage == "ranking")
        return sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.RankingPath", "./news/ranking.html");

    if (activePage == "news")
        return sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.NewsPath", "./news/news.html");

    return sConfigMgr->GetOption<std::string>("BreakingNews.HtmlPath", "./news/news.html");
}

bool TryReadNews(std::string& bn_Result)
{
    std::string path = GetZoeBreakingNewsPath();
    bn_Title = sConfigMgr->GetOption<std::string>("BreakingNews.Title", "ZoeCore News");
    ...
    if (!TryReadFile(path, bn_Result))
    {
        ...
        if (sConfigMgr->GetOption<bool>("BreakingNews.Zoe.FallbackToNews", true))
        {
            std::string fallbackPath = sConfigMgr->GetOption<std::string>("BreakingNews.Zoe.NewsPath", "./news/news.html");

            if (fallbackPath != path && TryReadFile(fallbackPath, bn_Result))
            {
                LOG_WARN("module", "ZoeCore BreakingNews: loaded fallback news page '{}'.", fallbackPath);
                return true;
            }
        }
    ...
    if (sConfigMgr->GetOption<bool>("BreakingNews.Zoe.Log.Enable", true))
        LOG_INFO("module", "ZoeCore BreakingNews: loaded page '{}'.", path);
...
void LoadBreakingNews()
{
    ...
    int32 frameX = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Frame.X", 16);
    int32 frameY = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Frame.Y", -160);
    uint32 frameWidth = sConfigMgr->GetOption<uint32>("BreakingNews.Zoe.Frame.Width", 350);
    uint32 frameHeight = sConfigMgr->GetOption<uint32>("BreakingNews.Zoe.Frame.Height", 535);

    int32 titleY = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Title.Y", -18);
    uint32 textSpacing = sConfigMgr->GetOption<uint32>("BreakingNews.Zoe.Text.Spacing", 4);

    int32 scrollTopLeftX = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Scroll.TopLeftX", 22);
    int32 scrollTopLeftY = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Scroll.TopLeftY", -44);
    int32 scrollBottomRightX = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Scroll.BottomRightX", -30);
    int32 scrollBottomRightY = sConfigMgr->GetOption<int32>("BreakingNews.Zoe.Scroll.BottomRightY", 34);

    uint32 textWidth = sConfigMgr->GetOption<uint32>("BreakingNews.Zoe.Frame.TextWidth", 300);
    uint32 textHeight = sConfigMgr->GetOption<uint32>("BreakingNews.Zoe.Frame.TextHeight", 405);

    bn_Formatted = Acore::StringFormat(_midPayloadFmt,
        frameX, frameY, frameWidth, frameHeight,
        titleY, textSpacing,
        scrollTopLeftX, scrollTopLeftY, scrollBottomRightX, scrollBottomRightY,
        textWidth, textHeight, bn_Title, bn_Body);
}
